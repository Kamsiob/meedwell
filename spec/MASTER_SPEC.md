...
- **No downloading from Bandcamp**, because the API offers no way. `READ_MEDIA_AUDIO` is what lets Meedwell see the music already on the phone and the files the user downloaded from Bandcamp themselves. `MediaStore` writes using `RELATIVE_PATH` and `IS_PENDING` remain the mechanism for anything Meedwell does place in the public Music folder, and they set the minSdk.
