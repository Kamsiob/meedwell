...
It is for someone who already buys music, on purpose, and wants a player that treats those purchases as possessions rather than as another catalog to browse. The anti-Symfonium point is concrete rather than rhetorical: Symfonium encrypts its offline cache, so its downloads are hostage to the app. Meedwell holds nothing hostage. Verification found that Bandcamp's API will not release purchased files to any client, so Meedwell does not fetch them; it recognizes the plain files a user downloads from Bandcamp themselves, plays them from where they sit in the open, and leaves them readable by any other player and untouched by an uninstall. The claim was always about portable files rather than about who fetches them, and it survives intact.
...
- **The API exposes only the user's own collection.** There is no store or catalog search. This shapes the entire Search screen and is the single most important constraint on the app's scope.
...
- Compiled against API 37, targeting API 36, minSdk 29. Play requires API 36 from 31 August 2026, verified against the live requirements page on 15 August 2026. minSdk is set by a feature rather than by taste: `MediaStore` writes using `RELATIVE_PATH` and `IS_PENDING` do not exist before API 29.
...
- **No downloading from Bandcamp**, because the API offers no way. Owned files reach the shelf through the Storage Access Framework instead: the user picks a folder, Meedwell takes a persistable grant on it, and `LocalScanner` reads it with `DocumentFile`. **That path needs no runtime permission**, which is why `READ_MEDIA_AUDIO` is no longer declared. The `MediaStore` write path described here previously was never built and cannot be, since there is nothing to download; it is withdrawn rather than deferred. See `DECISIONS.md`, 15 August 2026.
...
- Ambient color: `androidx.palette` plus `material-color-utilities`.
...
**Restore** replaces app data in one atomic operation after an explicit confirmation. Never a half import. Never silently drops data a version does not recognize: it says what it did not understand. Replace rather than merge is the deliberate choice, because merging two divergent listening histories is genuinely ambiguous, and the interface states that rather than guessing.
...
- **First sync.** Playable immediately, beta-honest copy, waiting tiles shimmering in the incoming album's own color rather than gray.
...
- **Permissions.** **One**, narrow, and asked for at the first note rather than at launch: notifications, only so the player appears in the shade and on the lock screen, never used for anything else. Refusing it leaves the app working with exactly one capability missing, stated plainly in Settings, where the live grant state is shown and a tap goes to the system page that can change it. Local folders need no permission at all and Settings says so. The previously specified second permission, music and audio files, is withdrawn: see `DECISIONS.md`, 15 August 2026.
- **Crash.** The local report shown in full, read before send, nothing auto-sent.

## 10. Testing protocol

Testing is continuous, not a final phase. Each phase has a gate that must pass before the next begins: unit tests for logic, instrumented tests for behavior, and a manual pass on the real device.
...
**American English, everywhere, without exception.** A standing order from the owner rather than a preference, given 15 August 2026 and recorded so a session starting cold does not drift back. It covers code comments, KDoc, every specification document, commit messages, issue text and all user-facing copy. The three deliberate exceptions are listed in `DESIGN.md` section 11.
...
The Play Console account is an organization account under the legal entity B7 Collective, public developer name **Kamsiob**, so B7 Collective is visible only if a user opens the about-the-developer view. The organization account was chosen deliberately because it avoids the closed-testing requirement personal accounts face.
